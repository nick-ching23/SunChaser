...
def do_task(request):
    ...
    try:
        ...
        command = f"docker run --rm {docker_img_path} --batch_size {request.batch} --start {request.start} --end {request.end} --output {request.id}_{request.start}_{request.end}_"
